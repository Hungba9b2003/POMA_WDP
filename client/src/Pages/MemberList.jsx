...
function MemberList() {
    ...
    const fetchProjectMembers = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const currentUser = data.memberInfo?.find(
          ...
          const uniqueMembers = Array.from(
            new Set(data.memberInfo.map((m) => m.id))
      ...
      if (result.isConfirmed) {
        try {
          const response = await fetch(
            `${API}/projects/${projectId}/member/${memberId}/delete`,
            {
              method: "DELETE",
          ...
          if (response.ok) {
            ...
            Swal.fire(
              "Đã xóa!",
              "Thành viên đã được xóa thành công.",
              "success"
            );
          } else {
  ...
  const handleSetRole = async (memberId, newRole) => {
    try {
      const response = await fetch(
        `${API}/projects/${projectId}/member/${memberId}/set-role`,
        {
          method: "PUT",
          headers: {
      ...
      if (response.ok) {
        ...
        Swal.fire(
          "Lỗi!",
          errorData.message || "Không thể cập nhật vai trò. Vui lòng thử lại.",
          "error"
        );
      }
    } catch (error) {
  ...
  const handleInviteMemberByEmail = async () => {
    ...
    try {
      const isAlreadyMember = projectMembers.filter(
