...
const TaskDetail = ({ task, showModal, onClose, onUpdateTask, isPremium }) => {
  ...
  const addComment = async () => {
    if (newComment) {
      try {
        await axios.post(
          `http://localhost:9999/projects/${projectId}/tasks/${task._id}/comments/create`,
          { content: newComment },
          {
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  const fetchProject = async () => {
    try {
      ...
      console.log(response.data.project.classifications);
      setStatusList(response.data.project.classifications);
    } catch (error) {
  ...
  const getSubtaskProgress = () => {
    ...
    const completed =
      (subTasks.filter((sub) => sub.status === "Completed").length / total) *
      100;
    const inProgress =
      (subTasks.filter((sub) => sub.status === "In Progress").length / total) *
      100;
    const pending =
      (subTasks.filter((sub) => sub.status === "Pending").length / total) * 100;
  ...
  const handleChangeReviewer = async (newReviewerId) => {
    ...
    const selectedReviewer = projectMembers.find(
      (member) => member.id === newReviewerId
    );
                            ...
                            {editingSubTaskId === subtask._id ? (
                              ...
                              >
                                ...
                                <button
                                  ...
                                  onClick={() => handleSaveSubTaskName(subtask)}
                                >
                                  ✔
                                </button>
                                <button
                                  ...
                                  onClick={handleCancelSubTaskName}
                                >
                                  ✖
                                </button>
                              </div>
                              ...
                              <h6
                                onClick={() => handleEditSubTaskName(subtask)}
                              >
                                {subtask.subTaskName}
                              </h6>
                            )}
                    ...
                    <li key={index} style={{ marginBottom: "20px" }}>
                      <Row className="align-items-center">
                        ...
                        <Col xs={1}>
                          <img
                            src={
                              c.user?.profile?.avatar || "default-avatar-url"
                            }
                            alt="Avatar"
                            style={{
                              width: "50px",
                        ...
                        <Col xs={11}>
                          ...
                          <Row>
                            ...
                            <Col xs={1} className="text-center">
                              {editingCommentId === c._id ? (
                                <button onClick={() => handleSave(c._id)}>
                                  Save
                                </button>
                              ) : (
                                <FaEdit
              ...
              <Col md={8}>
                <Dropdown>
                  <Dropdown.Toggle variant="link" id="dropdown-assignee">
                    ...
                    />
                    <span style={{ marginLeft: "10px" }}>{assigneeName}</span>
                  </Dropdown.Toggle>
                  <Dropdown.Menu>
                ...
                <Dropdown>
                  <Dropdown.Toggle variant="link" id="dropdown-reviewer">
                    <Image
                      ...
                      style={{ marginLeft: "10px", cursor: "pointer" }}
                    />
                    <span style={{ marginLeft: "10px" }}>{reviewerName}</span>
                  </Dropdown.Toggle>
                  <Dropdown.Menu>
                    {projectMembers.map((member) => (
                      <Dropdown.Item
